...
cv::Mat Scribbles::getAsImage(cv::Mat labelColors, Dim2 dim) const
{
    ...
    if (list_.size() != numLabels) {
        std::cout << "Requesting scribble image for " << numLabels << " labels, but current label count is " << list_.size();
        return cv::Mat();
    }
    ...
    for (uint i = 0; i < list_.size(); ++i) {
        for (uint j = 0; j < list_[i].size(); ++j) {
            ...
            if (x >= dim.width || y >= dim.height) {
                std::cout << "Scribble (" << x << "," << y << ") out of bounds for image dimensions (" << dim.width << "," << dim.height << ")";
                continue;
            }
            if (labelColors.at<cv::Vec3f>(0, i) == cv::Vec3f(1.0,1.0,1.0)) {
                std::cout << "Label color for label " << i << " is white. This info will be lost when saving the scribble image.";
                continue;
            }
...
{    
    if (scribbleImage.type() != CV_8UC3) {
        std::cout << "Expected type CV_8UC3 for scribble image, got " << matTypeToStr(scribbleImage.type());
        return;
    }

    if (dim.width != (uint)scribbleImage.cols || dim.height != (uint)scribbleImage.rows) {
        std::cout << "Expected scribble image size of (" << dim.width << "," << dim.height << "), but got size (" << scribbleImage.cols << "," << scribbleImage.rows << ")";
        return;
    }
        ...
        for (int y = 0; y < scribbleImage.rows; ++y) {
            ...
            if (val != triple_t(255,255,255)) {
                ...
                if (iter == reverseLabels.end()) {
                    std::cout << "Scribble color " << val << " at pixel (" << x << "," << y << ") does not match current label color... ignoring";
                    continue;
                }
                uint i = iter->second;
...
void Scribbles::saveScribbleImage(std::string filename, cv::Mat labelColors, Dim2 dim) const
{
    ...
    printf("Saving scribble image '%s' (%s %dx%d)", filename.c_str(), type.c_str(), image.cols, image.rows);
...
void Scribbles::loadScribbleImage(std::string filename, cv::Mat labelColors, Dim2 dim)
{
    ...
    std::string type = matTypeToStr(image.type());
    printf("Loaded scribble image '%s' (%s %dx%d)", filename.c_str(), type.c_str(), image.cols, image.rows);
